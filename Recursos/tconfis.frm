...
Begin VB.Form tconfis 
   BackColor       =   &H00FFFFC0&
   Caption         =   "Control Cierre Inventario"
   ClientHeight    =   10710
   ClientLeft      =   165
   ClientTop       =   15
   ClientWidth     =   15240
...
If opcion1 = "1" Then  'bodega
   If Len(buffer) = 0 Then
      cad = "SELECT * from seccion    "
   End If
   If Len(buffer) > 0 Then
      cad = "SELECT *  from seccion   where  " & Combo1 & " like '" & buffer & "%'"
   End If
...
If Frame2.Caption = "Nuevo" Then
   If Len(Seccion) = 0 Then
      ...
      Exit Function
   End If
   rbusca.Open "select seccion from seccion where seccion='" & Seccion & "'", cn, adOpenStatic, adLockOptimistic
