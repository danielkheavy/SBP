...
Begin VB.Form tplape 
   BackColor       =   &H00C0C0C0&
   Caption         =   "Periodos de Planilla"
   ClientHeight    =   10080
   ClientLeft      =   165
   ClientTop       =   -615
   ClientWidth     =   15240
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("periodo"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from plaperiodo    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from plaperiodo   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    periodo = Trim("" & txempre.Fields("periodo"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("periodo") = Trim(periodo)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        'If Val(Mid$(periodo, 1, 2)) < 1 And Val(Mid$(periodo, 1, 2)) > 12 Then
        '   periodo.SetFocus
        '   Exit Function
        'End If
        'If Val(Mid$(periodo, 3, 4)) < 2000 And Val(Mid$(periodo, 3, 4)) > 3000 Then
        '   periodo.SetFocus
        '   Exit Function
        'End If
        rbusca.Open "select periodo from plaperiodo where periodo='" & periodo & "'", cn, adOpenStatic, adLockOptimistic
        ...
        txempre.AddNew
        txempre.Fields("periodo") = periodo
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
