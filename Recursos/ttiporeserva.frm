...
Begin VB.Form ttiporeserva 
   ...
   End
End
Attribute VB_Name = "ttiporeserva"
...
Private Sub ajdu1_Click()
    ...
    habilita 1
    tiporeserva.Enabled = True
    tiporeserva = ""
    tiporeserva.SetFocus
...
Private Sub bo712_Click()
    ...
    buf = txempre.Fields("tiporeserva")
    ...
    If MsgBox("Desea Borra " + txempre.Fields("tiporeserva"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Private Sub dk9893_Click()
    ...
    reporgen.NAMETABLA = "tiporeserva"
    reporgen.Show 1
...
Sub prueba_reporte()
    ...
    'Call Reporter(CrystalReport1, globalpath & "\001d\06\reporte\tiporeservaesproducto.rpt", "")
End Sub

Private Sub tiporeserva_KeyPress(KeyAscii As Integer)
    ...
    If Len(tiporeserva) = 0 Then Exit Sub
    descripcio.SetFocus
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from tiporeserva    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from tiporeserva   where  " & Combo1 & " like '" & buffer & "%'"
...
Private Sub dbgrid1_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = 13 Then
        ...
        'tiporeserva = dbGrid1.Columns(1)
        'Frame1.Visible = False
        'Frame1.Enabled = False
        'tiporeserva.SetFocus
        'tiporeserva_KeyPress 13
    End If
...
Sub pone_registro()
    tiporeserva = Trim("" & txempre.Fields("tiporeserva"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("tiporeserva") = Trim(tiporeserva)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        If Len(tiporeserva) = 0 Then
            tiporeserva.SetFocus
            Exit Function

        End If

        rbusca.Open "select tiporeserva from tiporeserva where tiporeserva='" & tiporeserva & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe tiporeserva ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("tiporeserva") = tiporeserva
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
...
Sub agregar_menus()
    ...
    mytablex.Open "select * from archivo where menu='tiporeserva' and   estado='S'", cn, adOpenStatic, adLockOptimistic
...
Sub mnuarchivoarray_click(Index As Integer)
    ...
    mytablex.Open "select * from archivo where menu='tiporeserva' and descripcio='" & Trim(buf) & "'", cn, adOpenStatic, adLockOptimistic
