...
Begin VB.Form tdpreasi 
   BackColor       =   &H00FFFFC0&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Asientos Contables Predefinidos"
   ClientHeight    =   10230
   ClientLeft      =   150
   ClientTop       =   0
   ClientWidth     =   14745
   ...
   Begin VB.Frame Frame2 
      ...
      Begin MSDBGrid.DBGrid DBGrid2 
         Bindings        =   "tdpreasi.frx":2BFC
         Height          =   3975
         Left            =   120
         OleObjectBlob   =   "tdpreasi.frx":2C10
         TabIndex        =   38
         Top             =   3960
         Width           =   14415
      End
...
If MsgBox("Desea Borra " + "" & txasiento.Fields("cod_asien"), 1, "Aviso") <> 1 Then
   Exit Sub
End If
cn.Execute ("delete from dpreasiento where preasiento='" & preasiento & "' and cod_asien=" & txasiento.Fields("cod_asien"))
Command1_Click
Exit Sub
cmd656_err:
...
cad = "SELECT *  from dpreasiento   where  "
cad = cad & " preasiento='" & "" & preasiento & "'"
cad = cad & " and fecha_asi>='" & Format(FECHAI, "YYYYMMDD") & "'"
...
End Sub
Sub grabando()
txasiento.Fields("preasiento") = Trim(preasiento)
...
If Frame2.Caption = "Nuevo" Then
   ...
   '   Exit Function
   'End If
   sdx = 1 ' Val("" & mytablex.Fields("asientos")) + 1
'amiga:
   cod_asien = "" & sdx
   'rbusca.Open "select cod_asien from dpreasiento where cod_asien=" & cod_asien & "", cn, adOpenStatic, adLockOptimistic
   ...
   If Len("" & Data1.Recordset.Fields("cuenta")) > 0 Then
      txasiento.AddNew
      txasiento.Fields("cod_asien") = Val(cod_asien)
      grabando
      txasiento.Fields("fecha_asi") = Format(fecha, "dd/mm/yyyy")
...
If Frame2.Caption = "Modifica" Then
   ejecuta 1
   cn.Execute ("delete from dpreasiento where cod_asien='" & cod_asien & "'")
   Data1.refresh
   Do
   If Data1.Recordset.EOF Then Exit Do
...
xcredito = 0
If mytablex.State = 1 Then mytablex.Close
mytablex.Open "select * from dpreasiento where cod_asien='" & txasiento("cod_asien") & "'", cn, adOpenDynamic, adLockOptimistic
If mytablex.RecordCount = 0 Then
   mytablex.Close
   Exit Sub
End If
...
Dim mytablex As New ADODB.Recordset
mytablex.Open "select * from dpreasiento where preasiento='" & preasiento & "' and cod_asien=" & buf & "", cn, adOpenStatic, adLockOptimistic
If mytablex.RecordCount = 0 Then
   mytablex.Close
   Exit Sub
End If
...
If Frame3.Visible = True Then Exit Sub
If Frame2.Visible = True Then Exit Sub
reporgen.NAMETABLA = "dpreasiento"
reporgen.Show 1
