...
Begin VB.Form tprocedencia 
   ...
   End
End
Attribute VB_Name = "tprocedencia"
...
Private Sub bo712_Click()
    ...
    buf = txempre.Fields("procedencia")
    ...
    If MsgBox("Desea Borra " + txempre.Fields("procedencia"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Private Sub procedencia_KeyPress(KeyAscii As Integer)
    ...
    If Len(procedencia) = 0 Then Exit Sub
    descripcio.SetFocus
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from procedencia    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from procedencia   where  " & Combo1 & " like '" & buffer & "%'"
...
Private Sub djuer1_Click()
    ...
    reporgen.NAMETABLA = "procedencia"
    reporgen.Show 1
...
Sub pone_registro()
    procedencia = Trim("" & txempre.Fields("procedencia"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("procedencia") = Trim(procedencia)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select procedencia from procedencia where procedencia='" & procedencia & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe procedencia ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("procedencia") = procedencia
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
