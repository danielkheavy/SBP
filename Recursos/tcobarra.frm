...
Begin VB.Form tcobarra 
   BackColor       =   &H00C0C0C0&
   Caption         =   "Tabla de Codigo Barras"
   ClientHeight    =   9030
   ClientLeft      =   165
   ClientTop       =   15
   ClientWidth     =   13245
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("cobarra"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub prueba_reporte()
    ...
    'Call Reporter(CrystalReport1, globalpath & "\001d\06\reporte\cobarraesproducto.rpt", "")
End Sub

Private Sub cobarra_KeyPress(KeyAscii As Integer)
    ...
    If Len(cobarra) = 0 Then Exit Sub
    descripcio.SetFocus
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from cobarra    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from cobarra   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    cobarra = Trim("" & txempre.Fields("cobarra"))
    descripcio = Trim("" & txempre.Fields("descripcio"))
    Comando = Trim("" & txempre.Fields("comando"))

End Sub

Sub grabando()
    txempre.Fields("comando") = Trim(Comando)
    txempre.Fields("cobarra") = Trim(cobarra)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        If Len(cobarra) = 0 Then
            cobarra.SetFocus
            Exit Function

        End If

        rbusca.Open "select cobarra from cobarra where cobarra='" & cobarra & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe cobarra ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("cobarra") = cobarra
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
...
Sub agregar_menus()
    ...
    mytablex.Open "select * from archivo where menu='cobarra' and   estado='S'", cn, adOpenStatic, adLockOptimistic
...
Sub mnuarchivoarray_click(Index As Integer)
    ...
    mytablex.Open "select * from archivo where menu='cobarra' and descripcio='" & Trim(buf) & "'", cn, adOpenStatic, adLockOptimistic
