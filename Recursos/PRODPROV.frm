...
Begin VB.Form prodprov 
   BackColor       =   &H00FFFFC0&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Proveedores x Producto"
   ClientHeight    =   10710
   ClientLeft      =   150
   ClientTop       =   0
   ClientWidth     =   15240
...
buf = txempre.Fields("color")
If Frame2.Visible = True Then
   dbGrid1.SetFocus
   Exit Sub
End If
If MsgBox("Desea Borra " + txempre.Fields("color"), 1, "Aviso") <> 1 Then
   Exit Sub
End If
txempre.Delete
Command1_Click
...
If opcion1 = "1" Then  'bodega
   ...
   End If
   If Len(buffer) > 0 Then
      cad = "SELECT *  from color   where  " & Combo1 & " like '" & buffer & "%'"
   End If
...
End Sub
Sub pone_registro()
Color = Trim("" & txempre.Fields("color"))
descripcio = Trim("" & txempre.Fields("descripcio"))
End Sub
Sub grabando()
txempre.Fields("color") = Trim(Color)
...
If Frame2.Caption = "Nuevo" Then
   If Len(Color) = 0 Then
      ...
      Exit Function
   End If
   rbusca.Open "select color from color where color='" & Color & "'", cn, adOpenStatic, adLockOptimistic
   If rbusca.RecordCount > 0 Then
      rbusca.Close
      MsgBox "Ya existe color ", 48, "Aviso"
      Exit Function
   End If
   txempre.AddNew
   txempre.Fields("color") = Color
   grabando
   txempre.Update
   dlo132_Click
   Exit Function
