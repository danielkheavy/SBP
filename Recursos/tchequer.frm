...
Begin VB.Form tchequer 
   BackColor       =   &H00FFFF00&
   Caption         =   "Tabla de Chequeras"
   ClientHeight    =   6675
   ClientLeft      =   165
   ClientTop       =   555
   ClientWidth     =   11580
   ...
   Begin VB.Frame Frame1 
      ...
      Begin MSDBGrid.DBGrid DBGrid1 
         Bindings        =   "tchequer.frx":0000
         Height          =   5295
         Left            =   120
         OleObjectBlob   =   "tchequer.frx":0014
         TabIndex        =   23
         TabStop         =   0   'False
         Top             =   1080
         Width           =   11055
...
If opcion1 = "1" Then
   ...
   buf = "select Descripcio,Cuenta,Banco from tabbanco "
   Else
   buf = "select Descripcio,Cuenta,Banco from tabbanco  where " & Combo1 & " like '" & buffer & "*'"
   End If
...
If opcion1 = "2" Then
   If Len(buffer) = 0 Then
   buf = "select Descripcio,Chequera,Cuenta,Banco from chequera where cuenta='" & cuenta & "'"
   Else
   buf = "select Descripcio,Chequera,Cuenta,Banco from chequera where cuenta='" & cuenta & "' and " & Combo1 & " like '" & buffer & "*'"
   End If
   If Combo2.ListIndex = 1 Then
      buf = buf & " order by " & Combo1
   End If
...
Combo1.AddItem "chequera"
Combo1.ListIndex = 0
tipo.Clear
tipo.AddItem "NORMAL"
tipo.AddItem "DIFERIDO"
tipo.ListIndex = 0
End Sub
Sub inicializa()
descripcio = ""
...
Set mytablex = mydbx.OpenTable("chequera")
mytablex.Index = "chequera"
mytablex.Seek "=", cuenta, codigo
...
codigo = "" & mytablex.Fields("chequera")
cantidad = "" & mytablex.Fields("cantidad")
descripcio = "" & mytablex.Fields("descripcio")
tipo.ListIndex = 0
If "" & mytablex.Fields("tipo") = "NORMAL" Then
   tipo.ListIndex = 0
End If
If "" & mytablex.Fields("tipo") = "DIFERIDO" Then
   tipo.ListIndex = 1
End If
End Sub
...
mytablex.Fields("chequera") = codigo
...
banco = ""
Set mydbx = OpenDatabase(globaldir, False, False, "foxpro 2.5;")
Set mytablex = mydbx.OpenTable("tabbanco")
mytablex.Index = "tabbanco"
mytablex.Seek "=", banco, cuenta
If Not mytablex.NoMatch Then
   banco = "" & mytablex.Fields("banco")
End If
'------------------------------------- ------------
