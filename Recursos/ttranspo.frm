...
Begin VB.Form ttranspo 
   BackColor       =   &H00808080&
   Caption         =   "Tabla de Transportistas"
   ClientHeight    =   9930
   ClientLeft      =   165
   ClientTop       =   -765
   ClientWidth     =   15240
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from transpor    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from transpor   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    vehiculo = Trim("" & txempre.Fields("vehiculo"))
    marca = Trim("" & txempre.Fields("marca"))
    placa = Trim("" & txempre.Fields("placa"))
    licencia = Trim("" & txempre.Fields("licencia"))
...
Sub grabando()
    ...
    txempre.Fields("vehiculo") = Trim(vehiculo)
    txempre.Fields("marca") = Trim(marca)
    txempre.Fields("placa") = Trim(placa)
    txempre.Fields("licencia") = Trim(licencia)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select codigo from transpor where codigo='" & codigo & "'", cn, adOpenStatic, adLockOptimistic
