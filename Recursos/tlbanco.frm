...
Begin VB.Form tlbanco 
   BackColor       =   &H00808080&
   Caption         =   "Tabla de Cuentas de banco"
   ClientHeight    =   9930
   ClientLeft      =   165
   ClientTop       =   -765
   ClientWidth     =   15240
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("cuenta"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from tabbanco where banco='" & banco & "'"

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from tabbanco   where banco='" & banco & "' and " & Combo1 & " like '" & buffer & "%'"
...
Private Sub dbgrid1_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = 13 Then
        ...
        'tabbancol = dbGrid1.Columns(1)
        'Frame1.Visible = False
        'Frame1.Enabled = False
        'tabbancol.SetFocus
        'tabbancol_KeyPress 13
    End If
...
Sub grabando()
    ...
    txempre.Fields("cuenta") = Trim(cuenta)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select cuenta from tabbanco where banco='" & banco & "' and cuenta='" & cuenta & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe tabbancol ", 48, "Aviso"
            Exit Function
