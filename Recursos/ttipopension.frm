...
Begin VB.Form ttipopension 
   ...
   End
End
Attribute VB_Name = "ttipopension"
...
Private Sub ajdu1_Click()
    ...
    habilita 1
    tipopension.Enabled = True
    tipopension = ""
    tipopension.SetFocus
...
Private Sub bo712_Click()
    ...
    buf = txempre.Fields("tipopension")
    ...
    If MsgBox("Desea Borra " + txempre.Fields("tipopension"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Private Sub dk9893_Click()
    ...
    reporgen.NAMETABLA = "tipopension"
    reporgen.Show 1
...
Sub prueba_reporte()
    ...
    'Call Reporter(CrystalReport1, globalpath & "\001d\06\reporte\tipopensionesproducto.rpt", "")
End Sub

Private Sub tipopension_KeyPress(KeyAscii As Integer)
    ...
    If Len(tipopension) = 0 Then Exit Sub
    descripcio.SetFocus
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from tipopension    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from tipopension   where  " & Combo1 & " like '" & buffer & "%'"
...
Private Sub dbgrid1_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = 13 Then
        ...
        'tipopension = dbGrid1.Columns(1)
        'Frame1.Visible = False
        'Frame1.Enabled = False
        'tipopension.SetFocus
        'tipopension_KeyPress 13
    End If
...
Sub pone_registro()
    tipopension = Trim("" & txempre.Fields("tipopension"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("tipopension") = Trim(tipopension)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        If Len(tipopension) = 0 Then
            tipopension.SetFocus
            Exit Function

        End If

        rbusca.Open "select tipopension from tipopension where tipopension='" & tipopension & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe tipopension ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("tipopension") = tipopension
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
...
Sub agregar_menus()
    ...
    mytablex.Open "select * from archivo where menu='tipopension' and   estado='S'", cn, adOpenStatic, adLockOptimistic
...
Sub mnuarchivoarray_click(Index As Integer)
    ...
    mytablex.Open "select * from archivo where menu='tipopension' and descripcio='" & Trim(buf) & "'", cn, adOpenStatic, adLockOptimistic
