...
If Frame1.Caption = "NUEVO" Then
   rsexiste.Open "SELECT Turno FROM Turno where Turno='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
   If rsexiste.RecordCount > 0 Then  'si existe
      MsgBox "Ya existe codigo ", 48, "Aviso"
      codigo.SetFocus
      Exit Sub
   End If
   cad = "INSERT INTO Turno VALUES('" & Trim(codigo) & "','" & Trim(nombre) & "')"
   cn.Execute (cad)
   sql
   fdo33_Click
End If
If Frame1.Caption = "MODIFICA" Then
   cad = "UPDATE turno SET nombre = '" & Trim(nombre) & "' WHERE turno = '" & Trim(codigo) & "'"
   cn.Execute (cad)
   sql
   fdo33_Click
End If
...
If MsgBox("Desea Borrar " + dbgrid1.Columns(1), 1, "Aviso") = 1 Then
   cn.Execute ("DELETE   FROM turno WHERE turno ='" & Trim(dbgrid1.Columns(1)) & "'")
   rsede.Requery
   sql
End If
dbgrid1.SetFocus
...
rt.Open "SELECT * FROM turno ", cn, adOpenKeyset, adLockOptimistic
