...
Begin VB.Form tgrupo 
   BackColor       =   &H00FFFF00&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tabla Grupos"
   ClientHeight    =   8595
   ClientLeft      =   45
   ClientTop       =   615
   ClientWidth     =   10275
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Label Label4 
         BackColor       =   &H00FFFF80&
         Caption         =   "1.Inventario 2.Servicio"
         Height          =   495
         Left            =   1920
         TabIndex        =   16
         Top             =   1320
...
Dim cad As String
cad = "SELECT Nombre,grupo,Tipo FROM grupo "
If rsg.State = 1 Then rsg.Close
rsg.Open cad, cn, adOpenStatic, adLockOptimistic
Set dbgrid1.DataSource = rsg
...
If Frame1.Caption = "NUEVO" Then
   If tipo <> "1" And tipo <> "2" Then
      tipo = "1"
   End If
   rsexiste.Open "SELECT grupo FROM grupo where grupo='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
   If rsexiste.RecordCount > 0 Then  'si existe
      MsgBox "Ya existe codigo ", 48, "Aviso"
      codigo.SetFocus
      Exit Sub
   End If
   cad = "INSERT INTO grupo VALUES('" & Trim(codigo) & "','" & Trim(nombre) & "','" & Trim(tipo) & "')"
   cn.Execute (cad)
   sql
   fdo33_Click
End If
If Frame1.Caption = "MODIFICA" Then
   If tipo <> "1" And tipo <> "2" Then
      tipo = "1"
   End If
   cad = "UPDATE grupo SET nombre = '" & Trim(nombre) & "' , tipo= '" & Trim(tipo) & "' WHERE grupo = '" & Trim(codigo) & "'"
   cn.Execute (cad)
   sql
   fdo33_Click
End If
...
If MsgBox("Desea Borrar " + dbgrid1.Columns(1), 1, "Aviso") = 1 Then
   cn.Execute ("DELETE   FROM grupo WHERE grupo ='" & Trim(dbgrid1.Columns(1)) & "'")
   rsg.Requery
   sql
End If
dbgrid1.SetFocus
...
rt.Open "SELECT * FROM grupo ", cn, adOpenKeyset, adLockOptimistic
