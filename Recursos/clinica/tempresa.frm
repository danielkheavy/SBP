...
Begin VB.Form tempresa 
   BackColor       =   &H00FFFF00&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tabla Empresas"
   ClientHeight    =   8595
   ClientLeft      =   45
   ClientTop       =   615
   ClientWidth     =   10275
...
Dim cad As String
cad = "SELECT Nombre,empresa FROM empresa "
If rsemp.State = 1 Then rsemp.Close
rsemp.Open cad, cn, adOpenStatic, adLockOptimistic
Set dbgrid1.DataSource = rsemp
...
If Frame1.Caption = "NUEVO" Then
   rsexiste.Open "SELECT empresa FROM empresa where empresa='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
   If rsexiste.RecordCount > 0 Then  'si existe
      MsgBox "Ya existe codigo ", 48, "Aviso"
      codigo.SetFocus
      Exit Sub
   End If
   cad = "INSERT INTO empresa VALUES('" & Trim(codigo) & "','" & Trim(nombre) & "')"
   cn.Execute (cad)
   sql
   fdo33_Click
End If
If Frame1.Caption = "MODIFICA" Then
   cad = "UPDATE empresa SET nombre = '" & Trim(nombre) & "' WHERE empresa = '" & Trim(codigo) & "'"
   cn.Execute (cad)
   sql
   fdo33_Click
End If
...
If MsgBox("Desea Borrar " + dbgrid1.Columns(1), 1, "Aviso") = 1 Then
   cn.Execute ("DELETE   FROM empresa WHERE empresa ='" & Trim(dbgrid1.Columns(1)) & "'")
   rsemp.Requery
   sql
End If
dbgrid1.SetFocus
...
rt.Open "SELECT * FROM empresa ", cn, adOpenKeyset, adLockOptimistic
