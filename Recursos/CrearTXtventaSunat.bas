...
'Danielkheavy
'04/10/2018
'Extraer datos para enviar al servidor de datos InformaticaPeruana
Function estrae_ComprobanteAries(my_ruc As String, _
                                 local1 As String, _
                                 bxtipo As String, _
                                 bxserie As String, _
                                 bxnumero As String, _
    ...
    mysql = "SELECT f.paridad,f.usuario,f.vendedor, f.caja, f.SERIE," & Chr$(10)
    ...
    mysql = mysql & "f.local," & Chr$(10) ' se agreco el loca al SP SENDA alli llamado SUCU
    ...
    mysql = mysql & "f.descuento as tdescuentoFactura, " & Chr$(10) 'DescuentoItem
    mysql = mysql & "d.descuento as descuentoDetalle, " & Chr$(10) 'DescuentoItem
    mysql = mysql & "d.deslipo, " & Chr$(10) 'DescuentoPorcentaje descuento detalle
    '--------------
    'mysql = mysql & "d.destopo, " & Chr$(10) '% Descuento del Item
    '--------------
    ...
    mysql = mysql & "d.impuesto as impuestoDetalle," & Chr$(10) 'Impuesto Unitario IGV
    mysql = mysql & "d.comision as comision," & Chr$(10)
    mysql = mysql & "d.isc," & Chr$(10)
    mysql = mysql & "d.producto as codProducto," & Chr$(10)
    ...
    mysql = mysql & "and f.tipo='" & "" & bxtipo & "' " & Chr$(10)
    ...
    If mytable.RecordCount > 0 Then
        ...
        ' 'PARA LA SERIE
        '---------------------------------------------------------------------
        ' danielkheavy llenado de variables antes de ejecucion de Sp
        '---------------------------------------------------------------------
        varRUC_EMISOR = my_struc_datos_empresa(0).codigo1
        varRAZONSOCIAL_EMISOR = my_struc_datos_empresa(0).nombre
        varDIRECCIONFISCAL_EMISOR = my_struc_datos_empresa(0).direccion
        varTELEFONO_EMISOR = my_struc_datos_empresa(0).telefono
        varEMAIL_EMISOR = my_struc_datos_empresa(0).correo
        varTIPOVENTASUNAT = "01" ' VENTA INTERNA por defecto comentado en Modulo llamado SENDA_sp
        
        If mytable.Fields("tipo") = 1 Then
            varTIPODOCU = "03"
        ElseIf mytable.Fields("tipo") = 2 Then
            varTIPODOCU = "01"

        End If
             
        'Concatenamos y llenamos el valor de la serie y numero de la factura
        hastaCuanto = 8 - Len(mytable.Fields("NUMERO"))
        myDato = mytable.Fields("NUMERO")
        Call E_llenar_zero(hastaCuanto, myDato, xnumero)
        '----------------------------------------------------
        hora = Format(Now, "hh:mm:ss AM/PM")
        varNRO_EFACT = mytable.Fields("serie") & myDato 'asignacion de el valor despues del concatenar
        
        'NubeFact danielkheavy
        VarNubeSerie = mytable.Fields("serie")
        VarNubeNumero = mytable.Fields("NUMERO")
        'NubeFact danielkheavy
        
        varFEMISION = Now 'Fecha de emision
        varFVENC = Now  ' fecha de vencimiento
        varTIPODOCUREFE = "" 'TIPO DE DOCUMENTO DE REFERENCIA CUANDO ES UNA NOTA DE CREDITO O NOTA DE DEBITO
        varNUMEROREFE = "" 'NUMERO DE DOCUMENTO DE REFERENCIA CUANDO ES UNA NOTA DE CREDITO O NOTA DE DEBITO
        varCODIGO_MOTIVO_07_08 = "" ''CODIGO DEL MOTIVO CUANDO ES UNA NOTA DE CREDITO O NOTA DE DEBITO SEGÚN CATALOGO NRO 09 Y 10 SUNAT
        varDESCRIPCION_MOTIVO_07_08 = "" 'DESCRIPCION DEL MOTIVO CUANDO ES UNA NOTA DE CREDITO O NOTA DE DEBITO
        
        VarSerie = mytable.Fields("serie")
        VarNumer = mytable.Fields("NUMERO")
        '-------------------------------------------------------------
        '-------------------------------------------------------------
        
        VarClienteCodigo = Trim$(my_carga_busca_cliente(0).RUC)
        ...
        varCODIGO_CLIENTE = varNUMERODOI
        varNOMBRE_CLIENTE = my_carga_busca_cliente(0).nombre
        varEMAIL_FROM = my_carga_busca_cliente(0).correo
        varEMAIL_CC = my_struc_datos_empresa(0).correo
        
        varNOMB_VENDEDOR = mytable.Fields("vendedor")
        varNOMB_CAJERO = mytable.Fields("usuario")
        varCAJA = mytable.Fields("caja")
        
        varMETODO_PAGO = mytable.Fields("fpago")
        
        If "" & mytable.Fields("fpago") <> "Null" Then
            If mytable.Fields("fpago") = "3" Then
                varMETODO_PAGO = "CREDITO"
            Else
                varMETODO_PAGO = "CONTADO"
        ...
        varNRO_TRANSACCCION = ""
        varORDEN_COMPRA = ""
        varGLOSA = mytable.Fields("OBSERVA")
        varGLOSA_DETRACCION = ""
        varGLOSA_REFE = ""
        varMENSAJE = ""
        varGLOSA_PIE_PAGINA = ""
        ...
        If my_struc_datos_empresa(0).Toperacion = "E" Then
            varOP_EXONERADA = mytable.Fields("gravado")
            varOP_INAFECTA = 0
            varOP_GRAVADA = mytable.Fields("SUBTOTAL") - varOP_EXONERADA
        Else
            varOP_INAFECTA = mytable.Fields("gravado")
            varOP_EXONERADA = 0
            varOP_GRAVADA = mytable.Fields("SUBTOTAL") - varOP_INAFECTA

        End If
        
        varISC = 0
        varIGV = mytable.Fields("impuesto")
        varIMP_PERCEPCION = mytable.Fields("percepcion")
        varIMP_TOTAL = mytable.Fields("total")
        varTOTAL_PAGAR = mytable.Fields("total")
        
        varTOTAL_OTROS_TRIBUTOS = 0
        varTOTAL_OTROS_CARGOS = 0
        
        varTOTAL_DSCTO = mytable.Fields("tdescuentoFactura")
                
        varPORC_DSCTOGLOBAL = 0
        ' ----Solicitado por SENDA Descuento Global aplicar valor cero
        'varDSCTO_GLOBAL = mytable.Fields("tdescuentoFactura")
        varDSCTO_GLOBAL = 0
        '----------------------------
        varTOTAL_GRATUITAS = 0
        varTOTAL_ANTICIPO = 0
        varTIPO_MONEDA = mytable.Fields("moneda")

        If mytable.Fields("moneda") = "PEN" Then
            varIMPORTE_LETRAS = pone_letras("" & mytable.Fields("total"), "S", 0)
        ElseIf mytable.Fields("moneda") = "USD" Then
            varIMPORTE_LETRAS = pone_letras("" & mytable.Fields("total"), "D", 0)
        ...
        varVUELTO = Obtiene_Vuelto(local1, bxtipo, bxserie, bxnumero)

        varCON_DETALLE = 0
        varUSUARIO = mytable.Fields("usuario")
        varPORC_IGV = 18 ' asignado manualmente
        varTIPOCAMBIO = mytable.Fields("paridad")
        varSUCU = mytable.Fields("local")
        
        ' ok
        '---------------------------------------------------------------
        'LLamado del SP que insertara en la BD de SENDA
        'Call SENDAEFACT_INSERT_DOCUMENTOVENTA
        
        'Llamado Carga de variable TXT para envio con NubeFact
        '######################################################
        ...
        crear_txt_para_enviar = crear_txt_para_enviar & "total_gratuita|" & varTOTAL_GRATUITAS & "|" & vbcrlf & "total_otros_cargos|" & varTOTAL_OTROS_CARGOS & "|" & vbcrlf & "total|" & varTOTAL_PAGAR & "|" & vbcrlf & "percepcion_tipo||" & vbcrlf & "percepcion_base_imponible||" & vbcrlf & "total_percepcion|" & varIMP_PERCEPCION & "|" & vbcrlf & "total_incluido_percepcion||" & vbcrlf & "detraccion|false|" & vbcrlf & "observaciones||" & vbcrlf & "enviar_automaticamente_a_la_sunat|true|" & vbcrlf & "enviar_automaticamente_al_cliente|false|" & vbcrlf & "codigo_unico||" & vbcrlf & "condiciones_de_pago||" & vbcrlf & "medio_de_pago||" & vbcrlf & "placa_vehiculo||" & vbcrlf & "orden_compra_servicio||" & vbcrlf & "tabla_personalizada_codigo||" & vbcrlf & "formato_de_pdf||" & vbcrlf
        
        '####################################
        'Detalle de Comprobante de Pago
        '####################################
  
        Do
        
            If mytable.EOF Then Exit Do
            VarRegistroCount = mytable.RecordCount
                        
            '----------
            'Datos tomados de anterior proceso de asignacion de variables
            'varRUC_EMISOR
            'varTIPODOCU
            'varNRO_EFACT
            '-----------------------
            
            dvarINDICE = dvarINDICE + 1
            dvaris_anticipo = 0
            dvarCODIGO = mytable.Fields("codProducto")
            dvarNOMB_ITEM = mytable.Fields("descripcioP") ' Descripcion o nombre del producto
            dvarCANTIDAD = mytable.Fields("cantidad")
            dvarUM = mytable.Fields("unidad")
            dvarVALORUNITBRUTO = mytable.Fields("precio") / (mytable.Fields("igv") / 100 + 1)
            dvarVALOR_UNIT = mytable.Fields("precio") / (mytable.Fields("igv") / 100 + 1)
            dvarDSCTO_UNIT = mytable.Fields("descuentoDetalle")
            
            ' varTotal = dvarVALOR_UNIT - dvarDSCTO_UNIT
            
            dvarPORC_DESCTO = mytable.Fields("deslipo")
            dvarIGV_UNIT = mytable.Fields("impuestoDetalle")
            dvarISC = mytable.Fields("isc")
            dvarPRECIO_UNIT_BRUTO = mytable.Fields("precio")
            
            dvarPRECIO_UNIT = mytable.Fields("precio")
            
            dvarVALORVENTABRUTO = dvarCANTIDAD * dvarVALORUNITBRUTO 'mytable.Fields("subtotal")
            
            dvarVALOR_VENTA = dvarVALORVENTABRUTO
            dvarPRECIO_VENTA = (dvarCANTIDAD * mytable.Fields("precio")) - dvarDSCTO_UNIT
            
            '---Condiciones para tipo de afectacion IGV
            If mytable.Fields("igv") > 0 Then
                dvarTIPO_AFECT_IGV = "10"

            End If

            If mytable.Fields("igv") = 0 Then
                If varOP_INAFECTA > 0 Then
                    dvarTIPO_AFECT_IGV = "30"
                Else
                    dvarTIPO_AFECT_IGV = "20"
            ...
            'dvarTIPO_AFECT_IGV = "10" 'TIPO DE AFECTACION DEL IGV SEGUN CATALOGO NRO 7 SUNAT "Gravado operacion onerosa "
            
            dvarCODIGO_TRIBUTO = 1000 ' CODIGO DE TRIBUTO DEGUN CATALOGO NRO 5 SUNAT IGV  codigo "1000" Impuesto general a la ventas
            dvarTIPO_PRECIOVENTA = "01"  'TIPO DE PRECIO DE VENTA SEGUN CATALOGO NRO 16 SUNAT Precio Unitario incluye IGV
            dvarVISIBLE = 0 'SI ITEM SERA VISIBLE EN EL FORMATO DE IMPRESIÓN
            
            'Llamado a SP de Insercion en el Documento Detalle
            '----------------------------------------------------------
            'Call SENDAEFACT_INSERT_DOCUMENTOVENTADETALLE
            '----------------------------------------------------------
            crear_txt_para_enviarDetalle = "item|NIU|" & dvarCODIGO & " | " & dvarNOMB_ITEM & "|" & dvarCANTIDAD & "|" & dvarVALOR_UNIT & "|" & dvarPRECIO_VENTA & "||" & dvarIGV_UNIT & "|" & dvarVALORVENTABRUTO & "|" & 1 & "|" & dvarCANTIDAD & "|false|||" & vbcrlf
            crear_txt_para_enviar = crear_txt_para_enviar & crear_txt_para_enviarDetalle
            
            mytable.MoveNext
        Loop
        
        'Call SENDAEFACT_FIRMAR_XML_ARG
        'Call SENDAEFACT_GET_DSVALUE
        Call NUBEFACT_INSERT
        '-----------------------------------------
        'end danielkheavy proceso FE SENDA
        '-----------------------------------------
