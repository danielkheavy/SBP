...
If opcion1 = "1" Then  'bodega
   If Len(buffer) = 0 Then
      cad = "SELECT Descripcio,zona from zona    "
   End If
   If Len(buffer) > 0 Then
      cad = "SELECT Descripcio,zona from zona   where  " & Combo1 & " like '" & buffer & "%'"
   End If
   If rconsulta.State = 1 Then rconsulta.Close
   rconsulta.Open cad, cn, adOpenStatic, adLockOptimistic
...
cn.Execute ("DELETE   FROM zona WHERE zona='" & Trim(codigo) & "'")
borra_registro = 1
Exit Function
cmd56_err:
...
Dim rsexiste As New ADODB.Recordset
   rsexiste.Open "SELECT * FROM zona where  zona='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
...
If sw = 0 Then
   cad = "INSERT INTO zona VALUES('" & Trim(codigo) & "','"
...
If sw = 1 Then
   cad = "UPDATE zona SET "
   cad = cad & "zona = '" & Trim(codigo) & "'"
   cad = cad & ",descripcio='" & Trim(descripcio) & "'"
   cad = cad & " WHERE  zona='" & Trim(codigo) & "'"
   cn.Execute (cad)
