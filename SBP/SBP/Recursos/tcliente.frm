...
cn.Execute ("DELETE   FROM clientes WHERE codigo='" & Trim(codigo) & "'")
borra_registro = 1
Exit Function
cmd56_err:
...
Dim rsexiste As New ADODB.Recordset
   rsexiste.Open "SELECT * FROM clientes where  codigo='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
...
Dim rsexiste As New ADODB.Recordset
   rsexiste.Open "SELECT * FROM clientes where  codigo='" & Trim(buf) & "'", cn, adOpenKeyset, adLockOptimistic
   If rsexiste.RecordCount > 0 Then  'si existe
   If sw = 0 Then
