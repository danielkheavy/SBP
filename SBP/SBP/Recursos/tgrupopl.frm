...
Begin VB.Form tgrupopl 
   BackColor       =   &H00808080&
   Caption         =   "Grupos de Planilla"
   ClientHeight    =   10080
   ClientLeft      =   165
   ClientTop       =   -615
   ClientWidth     =   15240
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("grupo"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from grupopla   "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from grupopla   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    Grupo = Trim("" & txempre.Fields("grupo"))
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select grupo from grupopla where grupo='" & Grupo & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe grupo ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("grupo") = Grupo
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
