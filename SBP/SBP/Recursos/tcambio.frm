...
Begin VB.Form tcambio 
   BackColor       =   &H00C0C0C0&
   Caption         =   "Tabla de Tipo cambio Diario"
   ClientHeight    =   9225
   ClientLeft      =   165
   ClientTop       =   -45
   ClientWidth     =   13320
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("fecha"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from tcambio    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from tcambio   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    ...
    compra = Trim("" & txempre.Fields("compra"))
    venta = Trim("" & txempre.Fields("venta"))

End Sub

Sub grabando()
    txempre.Fields("fecha") = Trim(fecha)
    txempre.Fields("compra") = Val(compra)
    txempre.Fields("venta") = Val(venta)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select fecha from tcambio where fecha='" & fecha & "'", cn, adOpenStatic, adLockOptimistic
