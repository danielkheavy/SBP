...
Begin VB.Form tccosto 
   BackColor       =   &H00C0C0C0&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tabla de Centro de Costo"
   ClientHeight    =   7545
   ClientLeft      =   150
   ClientTop       =   120
   ClientWidth     =   13005
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("ccosto"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from ccosto    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from ccosto   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    ccosto = Trim("" & txempre.Fields("ccosto"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("ccosto") = Trim(ccosto)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select ccosto from ccosto where ccosto='" & ccosto & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe ccosto ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("ccosto") = ccosto
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
