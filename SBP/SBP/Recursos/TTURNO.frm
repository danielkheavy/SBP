...
Begin VB.Form tturno 
   BackColor       =   &H00808080&
   Caption         =   "Tabla de Turnos "
   ClientHeight    =   9930
   ClientLeft      =   165
   ClientTop       =   -765
   ClientWidth     =   15240
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.Label Label4 
         ...
         Caption         =   "0.Dia Anterior 1.MismoDia 2.DiaSiguiente"
         BeginProperty Font 
            Name            =   "Verdana"
            Size            =   8.25
            Charset         =   0
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("turno"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        ...
        If Len(buffer) > 0 Then
            cad = "SELECT *  from turno   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    FLAG = Trim("" & txempre.Fields("flag"))
    turno = Trim("" & txempre.Fields("turno"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("flag") = Trim(FLAG)
    txempre.Fields("turno") = Trim(turno)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select turno from turno where turno='" & turno & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe turno ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("turno") = turno
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
