...
If opcion1 = "1" Then  'bodega
   If Len(buffer) = 0 Then
      cad = "SELECT Descripcio,marca from marca    "
   End If
   If Len(buffer) > 0 Then
      cad = "SELECT Descripcio,marca from marca   where  " & Combo1 & " like '" & buffer & "%'"
   End If
   If rconsulta.State = 1 Then rconsulta.Close
   rconsulta.Open cad, cn, adOpenStatic, adLockOptimistic
...
cn.Execute ("DELETE   FROM marca WHERE marca='" & Trim(codigo) & "'")
borra_registro = 1
Exit Function
cmd56_err:
...
Dim rsexiste As New ADODB.Recordset
   rsexiste.Open "SELECT * FROM marca where  marca='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
...
If sw = 0 Then
   cad = "INSERT INTO marca VALUES('" & Trim(codigo) & "','"
...
If sw = 1 Then
   cad = "UPDATE marca SET "
   cad = cad & "marca = '" & Trim(codigo) & "'"
   cad = cad & ",descripcio='" & Trim(descripcio) & "'"
   cad = cad & " WHERE  marca='" & Trim(codigo) & "'"
   cn.Execute (cad)
