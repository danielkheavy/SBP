...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("servicio"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub prueba_reporte()
    ...
    'Call Reporter(CrystalReport1, globalpath & "\001d\06\reporte\servicioesproducto.rpt", "")
End Sub
...
Private Sub servicio_KeyPress(KeyAscii As Integer)
    ...
    If Len(servicio) = 0 Then Exit Sub
    descripcio.SetFocus
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from servicio    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from servicio   where  " & Combo1 & " like '" & buffer & "%'"
...
Private Sub dbgrid1_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = 13 Then
        ...
        'servicio = dbGrid1.Columns(1)
        'Frame1.Visible = False
        'Frame1.Enabled = False
        'servicio.SetFocus
        'servicio_KeyPress 13
    End If
...
Sub pone_registro()
    servicio = Trim("" & txempre.Fields("servicio"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("servicio") = Trim(servicio)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select servicio from servicio where servicio='" & servicio & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe servicio ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("servicio") = servicio
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
...
Sub agregar_menus()
    ...
    mytablex.Open "select * from archivo where menu='servicio' and   estado='S'", cn, adOpenStatic, adLockOptimistic
...
Sub mnuarchivoarray_click(Index As Integer)
    ...
    mytablex.Open "select * from archivo where menu='servicio' and descripcio='" & Trim(buf) & "'", cn, adOpenStatic, adLockOptimistic
