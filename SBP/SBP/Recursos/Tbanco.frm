...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("banco"), 1, "Aviso") <> 1 Then
        Exit Sub

    End If

    cn.Execute ("delete from tabbanco where banco='" & txempre.Fields("banco") & "'")
    txempre.Delete
    Command1_Click
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from banco    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from banco   where  " & Combo1 & " like '" & buffer & "%'"
...
Private Sub dju33881_Click()
    ...
    tlbanco.banco = "" & txempre.Fields("banco")
    tlbanco.nfamilia = "" & txempre.Fields("descripcio")
    tlbanco.Show 1
    Exit Sub
cmd4556_err:
    MsgBox "Seleccione un dato", 48, "Aviso"
...
Sub pone_registro()
    banco = Trim("" & txempre.Fields("banco"))
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select banco from banco where banco='" & banco & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe banco ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("banco") = banco
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
