...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("categoria"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Private Sub categoria_KeyPress(KeyAscii As Integer)
    ...
    If Len(categoria) = 0 Then Exit Sub
    descripcio.SetFocus
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from categori    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from categori   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    categoria = Trim("" & txempre.Fields("categoria"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("categoria") = Trim(categoria)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select categoria from categori where categoria='" & categoria & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe categoria ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("categoria") = categoria
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
