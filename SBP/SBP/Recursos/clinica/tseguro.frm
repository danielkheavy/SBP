...
Begin VB.Form tseguro 
   BackColor       =   &H00FFFF00&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tabla Seguros"
   ClientHeight    =   8595
   ClientLeft      =   45
   ClientTop       =   615
   ClientWidth     =   10275
...
Private Sub SQL()
    ...
    If rseg.State = 1 Then rseg.Close
    rseg.Open cad, cn, adOpenStatic, adLockOptimistic
    Set dbGrid1.DataSource = rseg
...
Private Sub Command3_Click()
    ...
    If Frame1.Caption = "NUEVO" Then
        rsexiste.Open "SELECT seguro FROM seguro where seguro='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
        ...
        cad = "INSERT INTO seguro VALUES('" & Trim(codigo) & "','" & Trim(nombre) & "')"
        cn.Execute (cad)
        SQL
        fdo33_Click
    ...
    If Frame1.Caption = "MODIFICA" Then
        cad = "UPDATE seguro SET nombre = '" & Trim(nombre) & "' WHERE seguro = '" & Trim(codigo) & "'"
        cn.Execute (cad)
        SQL
        fdo33_Click
...
Private Sub dfj8221_Click()
    ...
    If MsgBox("Desea Borrar " + dbGrid1.columns(1), 1, "Aviso") = 1 Then
        cn.Execute ("DELETE   FROM seguro WHERE seguro ='" & Trim(dbGrid1.columns(1)) & "'")
        rseg.Requery
        SQL
...
Private Sub dk281_Click()
    ...
    'rt.Open "SELECT * FROM seguro ", cn, adOpenKeyset, adLockOptimistic
