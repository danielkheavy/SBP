...
If Frame1.Caption = "NUEVO" Then
   rsexiste.Open "SELECT fpago FROM fpago where fpago='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
   If rsexiste.RecordCount > 0 Then  'si existe
      MsgBox "Ya existe codigo ", 48, "Aviso"
      codigo.SetFocus
      Exit Sub
   End If
   cad = "INSERT INTO fpago VALUES('" & Trim(codigo) & "','" & Trim(nombre) & "','" & Trim(moneda) & "')"
   cn.Execute (cad)
   sql
   fdo33_Click
End If
If Frame1.Caption = "MODIFICA" Then
   cad = "UPDATE fpago SET nombre = '" & Trim(nombre) & "',moneda='" & Trim(moneda) & "' WHERE fpago = '" & Trim(codigo) & "'"
   cn.Execute (cad)
   sql
   fdo33_Click
End If
...
If MsgBox("Desea Borrar " + dbgrid1.Columns(1), 1, "Aviso") = 1 Then
   cn.Execute ("DELETE   FROM fpago WHERE fpago ='" & Trim(dbgrid1.Columns(1)) & "'")
   rsfe.Requery
   sql
End If
dbgrid1.SetFocus
...
rt.Open "SELECT * FROM fpago ", cn, adOpenKeyset, adLockOptimistic
