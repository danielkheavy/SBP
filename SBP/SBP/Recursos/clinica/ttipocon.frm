...
Private Sub SQL()
    ...
    rstc.Open cad, cn, adOpenStatic, adLockOptimistic
    Set dbGrid1.DataSource = rstc
...
Private Sub Command3_Click()
    ...
    If Frame1.Caption = "NUEVO" Then
        rsexiste.Open "SELECT tipoconsu FROM tipoconsu where tipoconsu='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
        ...
        cad = "INSERT INTO tipoconsu VALUES('" & Trim(codigo) & "','" & Trim(nombre) & "')"
        cn.Execute (cad)
        SQL
        fdo33_Click
    ...
    If Frame1.Caption = "MODIFICA" Then
        cad = "UPDATE tipoconsu SET nombre = '" & Trim(nombre) & "' WHERE tipoconsu = '" & Trim(codigo) & "'"
        cn.Execute (cad)
        SQL
        fdo33_Click
...
Private Sub dfj8221_Click()
    ...
    If MsgBox("Desea Borrar " + dbGrid1.columns(1), 1, "Aviso") = 1 Then
        cn.Execute ("DELETE   FROM tipoconsu WHERE tipoconsu ='" & Trim(dbGrid1.columns(1)) & "'")
        rstc.Requery
        SQL
...
Private Sub dk281_Click()
    ...
    'rt.Open "SELECT * FROM tipoconsu ", cn, adOpenKeyset, adLockOptimistic
