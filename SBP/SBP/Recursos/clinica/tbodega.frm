...
Begin VB.Form tbodega 
   BackColor       =   &H00FFFF00&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tabla Almacenes"
   ClientHeight    =   8595
   ClientLeft      =   45
   ClientTop       =   615
   ClientWidth     =   12315
...
Dim cad As String
cad = "SELECT Nombre,Bodega,sede FROM bodega "
...
If Frame1.Caption = "NUEVO" Then
   rsexiste.Open "SELECT sede,bodega FROM BODEGA where sede='" & sede & "' and bodega='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
   If rsexiste.RecordCount > 0 Then  'si existe
      MsgBox "Ya existe codigo ", 48, "Aviso"
      codigo.SetFocus
      Exit Sub
   End If
   cad = "INSERT INTO BODEGA VALUES('" & Trim(sede) & "','" & Trim(codigo) & "','" & Trim(nombre) & "')"
   cn.Execute (cad)
   sql
   fdo33_Click
End If
If Frame1.Caption = "MODIFICA" Then
   cad = "UPDATE BODEGA SET nombre = '" & Trim(nombre) & "' WHERE sede = '" & Trim(sede) & "' and bodega='" & Trim(codigo) & "'"
   cn.Execute (cad)
   sql
   fdo33_Click
End If
...
If MsgBox("Desea Borrar " + dbgrid1.Columns(1), 1, "Aviso") = 1 Then
   cn.Execute ("DELETE   FROM bodega WHERE sede ='" & Trim(dbgrid1.Columns(2)) & "' and bodega='" & Trim(dbgrid1.Columns(0)) & "'")
   rsede.Requery
   sql
End If
dbgrid1.SetFocus
