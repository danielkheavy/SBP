...
Begin VB.Form ttipoaut 
   BackColor       =   &H00C0C0C0&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tabla Tipos de Autorizaciones"
   ClientHeight    =   8595
   ClientLeft      =   45
   ClientTop       =   615
   ClientWidth     =   10275
...
Private Sub SQL()
    ...
    rsta.Open cad, cn, adOpenStatic, adLockOptimistic
    Set dbGrid1.DataSource = rsta
...
Private Sub Command3_Click()
    ...
    If Frame1.Caption = "NUEVO" Then
        rsexiste.Open "SELECT tipoautor FROM tipoautor where tipoautor='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
        ...
        cad = "INSERT INTO tipoautor VALUES('" & Trim(codigo) & "','" & Trim(nombre) & "')"
        cn.Execute (cad)
        SQL
        fdo33_Click
    ...
    If Frame1.Caption = "MODIFICA" Then
        cad = "UPDATE tipoautor SET nombre = '" & Trim(nombre) & "' WHERE tipoautor = '" & Trim(codigo) & "'"
        cn.Execute (cad)
        SQL
        fdo33_Click
...
Private Sub dfj8221_Click()
    ...
    If MsgBox("Desea Borrar " + dbGrid1.columns(1), 1, "Aviso") = 1 Then
        cn.Execute ("DELETE   FROM tipoautor WHERE tipoautor ='" & Trim(dbGrid1.columns(1)) & "'")
        rsta.Requery
        SQL
...
Private Sub dk281_Click()
    ...
    'rt.Open "SELECT * FROM tipoautor ", cn, adOpenKeyset, adLockOptimistic
