...
Begin VB.Form tenferme 
   BackColor       =   &H00808080&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tabla Enfermedades"
   ClientHeight    =   8595
   ClientLeft      =   45
   ClientTop       =   615
   ClientWidth     =   10275
...
Private Sub SQL()
    ...
    If rsfe.State = 1 Then rsfe.Close
    rsfe.Open cad, cn, adOpenStatic, adLockOptimistic
    Set dbGrid1.DataSource = rsfe
...
Private Sub Command3_Click()
    ...
    If Frame1.Caption = "NUEVO" Then
        rsexiste.Open "SELECT enfermedad FROM enfermedad where enfermedad='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
        ...
        cad = "INSERT INTO enfermedad VALUES('" & Trim(codigo) & "','" & Trim(nombre) & "')"
        cn.Execute (cad)
        SQL
        fdo33_Click
    ...
    If Frame1.Caption = "MODIFICA" Then
        cad = "UPDATE enfermedad SET nombre = '" & Trim(nombre) & "' WHERE enfermedad = '" & Trim(codigo) & "'"
        cn.Execute (cad)
        SQL
        fdo33_Click
...
Private Sub dfj8221_Click()
    ...
    If MsgBox("Desea Borrar " + dbGrid1.columns(1), 1, "Aviso") = 1 Then
        cn.Execute ("DELETE   FROM enfermedad WHERE enfermedad ='" & Trim(dbGrid1.columns(1)) & "'")
        rsfe.Requery
        SQL
...
Private Sub dk281_Click()
    ...
    'rt.Open "SELECT * FROM enfermedad ", cn, adOpenKeyset, adLockOptimistic
