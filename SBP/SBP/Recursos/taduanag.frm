...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("gasto"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub prueba_reporte()
    ...
    'Call Reporter(CrystalReport1, globalpath & "\001d\06\reporte\gastog", "")
End Sub
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from gastos    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from gastos   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    gasto = Trim("" & txempre.Fields("gasto"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("gasto") = Trim(gasto)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select gasto from gastos where gasto='" & gasto & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe gasto ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("gasto") = gasto
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
...
Sub agregar_menus()
    ...
    mytablex.Open "select * from archivo where menu='gasto' and   estado='S'", cn, adOpenStatic, adLockOptimistic
...
Sub mnuarchivoarray_click(Index As Integer)
    ...
    mytablex.Open "select * from archivo where menu='gasto' and descripcio='" & Trim(buf) & "'", cn, adOpenStatic, adLockOptimistic
