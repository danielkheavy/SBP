...
Begin VB.Form ttcentro 
   BackColor       =   &H00808080&
   Caption         =   "Tabla de CentroProduccion"
   ClientHeight    =   9930
   ClientLeft      =   165
   ClientTop       =   -765
   ClientWidth     =   18570
   ...
   Begin VB.Menu djuer1 
      ...
      End
   End
   Begin VB.Menu hyru8833 
      Caption         =   "&GrupoCentroProduccion"
   End
   Begin VB.Menu dlo132 
      Caption         =   "&Salir"
   End
End
Attribute VB_Name = "ttcentro"
...
Private Sub bo712_Click()
    ...
    buf = "" & txempre.Fields("centroproduccion")
    ...
    If MsgBox("Desea Borra " + txempre.Fields("centroproduccion"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Private Sub centroproduccion_KeyPress(KeyAscii As Integer)
    ...
    If Len(centroproduccion) = 0 Then Exit Sub
    descripcio.SetFocus
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from centroproduccion    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from centroproduccion   where  " & Combo1 & " like '" & buffer & "%'"
...
Private Sub Form_Load()
    ...
    Combo1.ListIndex = 0
    Label13 = "Son las areas o procesos de produccion(Corte Ensamblaje Soldadura)" & Chr$(10) & Chr$(13)
    Label13 = Label13 & "Debemos definir los tipos de recurso ,la tarifa dia (Valor por 8 horas )" & Chr$(10) & Chr$(13)
    Label13 = Label13 & "El Volumen Estimado de produccion en caso se pueda medir (Opcional)" & Chr$(10) & Chr$(13)
    Label13 = Label13 & "" & Chr$(10) & Chr$(13)
    Label13 = Label13 & "Mencionar las maquinas que participan, la tarifa dia (Valor de desgaste o depreciacion diaria)" & Chr$(10) & Chr$(13)
    Label13 = Label13 & "" & Chr$(10) & Chr$(13)
    Label13 = Label13 & "El Volumen estimado de produccion,si fuera posible determinar (Opcional)" & Chr$(10) & Chr$(13)
...
Sub pone_registro()
    centroproduccion = Trim("" & txempre.Fields("centroproduccion"))
    descripcio = Trim("" & txempre.Fields("descripcio"))
    activo = Trim("" & txempre.Fields("activo"))
    observa = Trim("" & txempre.Fields("observa"))

End Sub

Sub grabando()
    txempre.Fields("centroproduccion") = Trim(centroproduccion)
    txempre.Fields("descripcio") = Trim(descripcio)
    txempre.Fields("activo") = Trim(activo)
    txempre.Fields("observa") = Trim(observa)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        txempre.AddNew
        txempre.Fields("centroproduccion") = centroproduccion
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
...
Private Sub hyru8833_Click()

    On Error GoTo cmd9032_err

    tcentrop.centroproduccion = Trim(txempre.Fields("centroproduccion"))
    tcentrop.ncentroproduccion = Trim(txempre.Fields("descripcio"))
    tcentrop.Show 1
    Exit Sub
cmd9032_err:
    MsgBox "Seleccione un dato ", 48, "Aviso"
