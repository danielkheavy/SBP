...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("tipotarifa"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub prueba_reporte()
    ...
    'Call Reporter(CrystalReport1, globalpath & "\001d\06\reporte\tipotarifaesproducto.rpt", "")
End Sub

Private Sub tipotarifa_KeyPress(KeyAscii As Integer)
    ...
    If Len(tipotarifa) = 0 Then Exit Sub
    descripcio.SetFocus
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from tipotarifa    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from tipotarifa   where  " & Combo1 & " like '" & buffer & "%'"
...
Private Sub dbgrid1_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = 13 Then
        ...
        'tipotarifa = dbGrid1.Columns(1)
        'Frame1.Visible = False
        'Frame1.Enabled = False
        'tipotarifa.SetFocus
        'tipotarifa_KeyPress 13
    End If
...
Sub pone_registro()
    tipotarifa = Trim("" & txempre.Fields("tipotarifa"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("tipotarifa") = Trim(tipotarifa)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        If Len(tipotarifa) = 0 Then
            tipotarifa.SetFocus
            Exit Function

        End If

        rbusca.Open "select tipotarifa from tipotarifa where tipotarifa='" & tipotarifa & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe tipotarifa ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("tipotarifa") = tipotarifa
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
...
Sub agregar_menus()
    ...
    mytablex.Open "select * from archivo where menu='tipotarifa' and   estado='S'", cn, adOpenStatic, adLockOptimistic
...
Sub mnuarchivoarray_click(Index As Integer)
    ...
    mytablex.Open "select * from archivo where menu='tipotarifa' and descripcio='" & Trim(buf) & "'", cn, adOpenStatic, adLockOptimistic
