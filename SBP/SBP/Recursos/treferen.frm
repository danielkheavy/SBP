...
Begin VB.Form treferen 
   BackColor       =   &H00FFFF00&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tabla Referencias"
   ClientHeight    =   8595
   ClientLeft      =   45
   ClientTop       =   615
   ClientWidth     =   10275
...
If rsre.State = 1 Then rsre.Close
rsre.Open cad, cn, adOpenStatic, adLockOptimistic
Set dbGrid1.DataSource = rsre
...
If Frame1.Caption = "NUEVO" Then
   rsexiste.Open "SELECT referencia FROM referencia where referencia='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
   If rsexiste.RecordCount > 0 Then  'si existe
      MsgBox "Ya existe codigo ", 48, "Aviso"
      codigo.SetFocus
      Exit Sub
   End If
   cad = "INSERT INTO referencia VALUES('" & Trim(codigo) & "','" & Trim(nombre) & "')"
   cn.Execute (cad)
   SQL
   fdo33_Click
End If
If Frame1.Caption = "MODIFICA" Then
   cad = "UPDATE referencia SET nombre = '" & Trim(nombre) & "' WHERE referencia = '" & Trim(codigo) & "'"
   cn.Execute (cad)
   SQL
   fdo33_Click
End If
...
If MsgBox("Desea Borrar " + dbGrid1.columns(1), 1, "Aviso") = 1 Then
   cn.Execute ("DELETE   FROM referencia WHERE referencia ='" & Trim(dbGrid1.columns(1)) & "'")
   rsre.Requery
   SQL
End If
dbGrid1.SetFocus
...
'rt.Open "SELECT * FROM referencia ", cn, adOpenKeyset, adLockOptimistic
