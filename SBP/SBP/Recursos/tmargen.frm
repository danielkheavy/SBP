...
Begin VB.Form tmargen 
   BackColor       =   &H00808080&
   Caption         =   "Tabla de Rango Margenes"
   ClientHeight    =   9930
   ClientLeft      =   165
   ClientTop       =   -765
   ClientWidth     =   15240
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("margen"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from margen    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from margen   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    margen = Trim("" & txempre.Fields("margen"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("margen") = Trim(margen)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        If Len(margen) = 0 Then
            margen.SetFocus
            Exit Function

        End If

        rbusca.Open "select margen from margen where margen='" & margen & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe margen ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("margen") = margen
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
