...
Begin VB.Form tgrupoco 
   BackColor       =   &H00808080&
   Caption         =   "Tabla de Grupos de Comentarios"
   ClientHeight    =   9930
   ClientLeft      =   165
   ClientTop       =   -765
   ClientWidth     =   15120
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("grupoco"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub prueba_reporte()
    ...
    'Call Reporter(CrystalReport1, globalpath & "\001d\06\reporte\grupocoesproducto.rpt", "")
End Sub

Private Sub grupoco_KeyPress(KeyAscii As Integer)
    ...
    If Len(grupoco) = 0 Then Exit Sub
    descripcio.SetFocus
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from grupoco    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from grupoco   where  " & Combo1 & " like '" & buffer & "%'"
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        If Len(grupoco) = 0 Then
            grupoco.SetFocus
            Exit Function

        End If

        rbusca.Open "select grupoco from grupoco where grupoco='" & grupoco & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe grupoco ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("grupoco") = grupoco
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
...
Sub agregar_menus()
    ...
    mytablex.Open "select * from archivo where menu='grupoco' and   estado='S'", cn, adOpenStatic, adLockOptimistic
...
Sub mnuarchivoarray_click(Index As Integer)
    ...
    mytablex.Open "select * from archivo where menu='grupoco' and descripcio='" & Trim(buf) & "'", cn, adOpenStatic, adLockOptimistic
