...
Begin VB.Form thabita 
   BackColor       =   &H00FFFFC0&
   Caption         =   "Tabla de Habitaciones"
   ClientHeight    =   9135
   ClientLeft      =   165
   ClientTop       =   15
   ClientWidth     =   12540
...
buf = txempre.Fields("habitacion")
If Frame2.Visible = True Then
   dbGrid1.SetFocus
   Exit Sub
End If
If MsgBox("Desea Borra " + txempre.Fields("habitacion"), 1, "Aviso") <> 1 Then
   Exit Sub
End If
txempre.Delete
Command1_Click
...
End Sub
Sub prueba_reporte()
'Call Reporter(CrystalReport1, globalpath & "\001d\06\reporte\habitacionesproducto.rpt", "")
End Sub
...
If opcion1 = "1" Then  'bodega
   ...
   End If
   If Len(buffer) > 0 Then
      cad = "SELECT *  from habitacion   where  " & Combo1 & " like '" & buffer & "%'"
   End If
...
End Sub
Sub pone_registro()
habitacion = Trim("" & txempre.Fields("habitacion"))
descripcio = Trim("" & txempre.Fields("descripcio"))
End Sub
Sub grabando()
txempre.Fields("habitacion") = Trim(habitacion)
...
If Frame2.Caption = "Nuevo" Then
   If Len(habitacion) = 0 Then
      ...
      Exit Function
   End If
   rbusca.Open "select habitacion from habitacion where habitacion='" & habitacion & "'", cn, adOpenStatic, adLockOptimistic
   If rbusca.RecordCount > 0 Then
      rbusca.Close
      MsgBox "Ya existe habitacion ", 48, "Aviso"
      Exit Function
   End If
   txempre.AddNew
   txempre.Fields("habitacion") = habitacion
   grabando
   txempre.Update
   dlo132_Click
   Exit Function
...
Dim mytablex As New ADODB.Recordset
   mytablex.Open "select * from archivo where menu='habitacion' and   estado='S'", cn, adOpenStatic, adLockOptimistic
...
buf = mnuArchivoArray(Index).Caption
   mytablex.Open "select * from archivo where menu='habitacion' and descripcio='" & Trim(buf) & "'", cn, adOpenStatic, adLockOptimistic
   If mytablex.RecordCount = 0 Then
      mytablex.Close
   End If
   'busca el reporte
