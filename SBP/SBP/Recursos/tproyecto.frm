...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("proyecto"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        ...
        If Len(buffer) > 0 Then
            cad = "SELECT *  from proyecto   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    proyecto = Trim("" & txempre.Fields("proyecto"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("proyecto") = Trim(proyecto)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select proyecto from proyecto where proyecto='" & proyecto & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe proyecto ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("proyecto") = proyecto
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
