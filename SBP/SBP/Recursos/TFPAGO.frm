...
Begin VB.Form tfpago 
   BackColor       =   &H00808080&
   Caption         =   "Tabla de Formas de pago"
   ClientHeight    =   9105
   ClientLeft      =   165
   ClientTop       =   -45
   ClientWidth     =   12705
   ...
   Begin VB.Frame Frame2 
      ...
      Visible         =   0   'False
      Width           =   12375
      Begin VB.TextBox cuentacontable 
         BeginProperty Font 
            Name            =   "Verdana"
            Size            =   9.75
            Charset         =   0
      ...
      Begin VB.Label Label7 
         BackColor       =   &H00E0E0E0&
         BorderStyle     =   1  'Fixed Single
         Caption         =   "Cruce Documentos: I.CruceAdelantosDinero J.OrdenTrabajo U.Exonerado K.CruceDeposito Bancario"
         BeginProperty Font 
            Name            =   "Verdana"
            Size            =   9.75
            Charset         =   0
      ...
      Begin VB.Label Label6 
         BackColor       =   &H00E0E0E0&
         BorderStyle     =   1  'Fixed Single
         Caption         =   "Grupos:A.Efectivo B.Dolares C.Credito D.TarjetaCredito E.Euros F.TarjetaDebito G.Letra H.Banco  V.ales O.tros"
         BeginProperty Font 
            Name            =   "Verdana"
            Size            =   9.75
            Charset         =   0
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("fpago"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from fpago    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from fpago   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    ...
    vecaja = Trim("" & txempre.Fields("vecaja"))
    tipo = Trim("" & txempre.Fields("tipo"))

    'Reporte de ingresos (Cobranzas) CONTASIS
    cuentacontable = Trim("" & txempre.Fields("cuentacontable"))
    'Reporte de ingresos (Cobranzas) CONTASIS

End Sub

Sub grabando()
    txempre.Fields("fpago") = Trim(fpago)
    txempre.Fields("descripcio") = Trim(descripcio)
    txempre.Fields("moneda") = Trim(moneda)
    txempre.Fields("vecaja") = Trim(vecaja)
    txempre.Fields("tipo") = Trim(tipo)

    'Reporte de ingresos (Cobranzas) CONTASIS
    txempre.Fields("cuentacontable") = Trim(cuentacontable)
    'Reporte de ingresos (Cobranzas) CONTASIS
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select fpago from fpago where fpago='" & fpago & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe fpago ", 48, "Aviso"
            Exit Function
