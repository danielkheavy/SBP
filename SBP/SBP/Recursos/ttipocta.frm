...
Begin VB.Form ttipocta 
   BackColor       =   &H00C0C0C0&
   Caption         =   "Tabla de Tipos de Cuentas"
   ClientHeight    =   10080
   ClientLeft      =   165
   ClientTop       =   -615
   ClientWidth     =   15240
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("tipocta"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub prueba_reporte()
    ...
    'Call Reporter(CrystalReport1, globalpath & "\001d\06\reporte\tipoctaesproducto.rpt", "")
End Sub

Private Sub tipocta_KeyPress(KeyAscii As Integer)
    ...
    If Len(tipocta) = 0 Then Exit Sub
    descripcio.SetFocus
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from tipocta    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from tipocta   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    tipocta = Trim("" & txempre.Fields("tipocta"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("tipocta") = Trim(tipocta)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        If Len(tipocta) = 0 Then
            tipocta.SetFocus
            Exit Function

        End If

        rbusca.Open "select tipocta from tipocta where tipocta='" & tipocta & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe tipocta ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("tipocta") = tipocta
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
...
Sub agregar_menus()
    ...
    mytablex.Open "select * from archivo where menu='tipocta' and   estado='S'", cn, adOpenStatic, adLockOptimistic
...
Sub mnuarchivoarray_click(Index As Integer)
    ...
    mytablex.Open "select * from archivo where menu='tipocta' and descripcio='" & Trim(buf) & "'", cn, adOpenStatic, adLockOptimistic
