...
Begin VB.Form tdocta 
   BackColor       =   &H00808080&
   Caption         =   "Tabla de Tipos documentos"
   ClientHeight    =   9930
   ClientLeft      =   165
   ClientTop       =   -765
   ClientWidth     =   15240
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("docta"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub prueba_reporte()
    ...
    'Call Reporter(CrystalReport1, globalpath & "\001d\06\reporte\doctaesproducto.rpt", "")
End Sub

Private Sub docta_KeyPress(KeyAscii As Integer)
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from docta    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from docta   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    docta = Trim("" & txempre.Fields("docta"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("docta") = Trim(docta)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select docta from docta where docta='" & docta & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe docta ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("docta") = docta
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
...
Sub agregar_menus()
    ...
    mytablex.Open "select * from archivo where menu='docta' and   estado='S'", cn, adOpenStatic, adLockOptimistic
...
Sub mnuarchivoarray_click(Index As Integer)
    ...
    mytablex.Open "select * from archivo where menu='docta' and descripcio='" & Trim(buf) & "'", cn, adOpenStatic, adLockOptimistic
