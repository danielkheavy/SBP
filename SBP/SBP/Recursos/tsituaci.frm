...
Begin VB.Form tsituaci 
   BackColor       =   &H00808080&
   Caption         =   "Tabla de Situaciones"
   ClientHeight    =   9930
   ClientLeft      =   165
   ClientTop       =   -765
   ClientWidth     =   15240
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("situacion"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Private Sub situacion_KeyPress(KeyAscii As Integer)
    ...
    If Len(situacion) = 0 Then Exit Sub
    descripcio.SetFocus
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from situacion    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from situacion   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    situacion = Trim("" & txempre.Fields("situacion"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("situacion") = Trim(situacion)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select situacion from situacion where situacion='" & situacion & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe situacion ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("situacion") = situacion
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
