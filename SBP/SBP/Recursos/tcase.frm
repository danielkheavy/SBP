...
Begin VB.Form tcase 
   BackColor       =   &H00C0C0C0&
   Caption         =   "Tabla de Secciones Cartera"
   ClientHeight    =   8700
   ClientLeft      =   165
   ClientTop       =   15
   ClientWidth     =   13470
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("carsec"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Private Sub carsec_KeyPress(KeyAscii As Integer)
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from carsec    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from carsec   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    carsec = Trim("" & txempre.Fields("carsec"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("carsec") = Trim(carsec)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        If Len(carsec) = 0 Then
            carsec.SetFocus
            Exit Function

        End If

        rbusca.Open "select carsec from carsec where carsec='" & carsec & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe carsec ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("carsec") = carsec
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
