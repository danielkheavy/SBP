...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("talla"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from talla    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from talla   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    talla = Trim("" & txempre.Fields("talla"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("talla") = Trim(talla)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select talla from talla where talla='" & talla & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe talla ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("talla") = talla
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
