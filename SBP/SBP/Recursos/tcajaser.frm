...
Begin VB.Form tcajaser 
   BackColor       =   &H00FFFFC0&
   Caption         =   "Tabla de Series"
   ClientHeight    =   6630
   ClientLeft      =   165
   ClientTop       =   135
   ClientWidth     =   12555
...
If opcion1 = "1" Then  'bodega
   cad = "SELECT * from seriedoc   where caja='" & caja & "'"
...
serie = Trim("" & txempre.Fields("serie"))

numero = Trim("" & txempre.Fields("numero"))
puerto = Trim("" & txempre.Fields("puerto"))
cola = Trim("" & txempre.Fields("cola"))
archivo = Trim("" & txempre.Fields("archivo"))
iconfirma = Trim("" & txempre.Fields("iconfirma"))
...
txempre.Fields("iconfirma") = Trim(iconfirma)
...
If Frame2.Caption = "Nuevo" Then
   ...
   If Len(serie) = 0 Then
      ...
      Exit Function
   End If
   rbusca.Open "select tipo from seriedoc where caja='" & caja & "' and tipo='" & tipo & "' and serie='" & serie & "'", cn, adOpenStatic, adLockOptimistic
