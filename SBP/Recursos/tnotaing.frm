...
Private Sub bo712_Click()
    ...
    'If MsgBox("Desea Borra " + "" & txformxu.Fields("numero"), 1, "Aviso") <> 1 Then
    '   Exit Sub
    'End If
    'borrar_notaingreso
    txformxu.Delete
...
Private Sub Command2_Click()
    ...
    dnotaing.idx = "" & txformxu.Fields("numero")
    dnotaing.bodega = "" & txformxu.Fields("bodega")
    dnotaing.ordentrabajo = "" & txformxu.Fields("ordentrabajo")
    dnotaing.tipomov = "" & txformxu.Fields("tipomov")

    If tipomov = "S" Then
        dnotaing.Caption = "Nota Entrada"

    End If

    If tipomov = "T" Then
        dnotaing.Caption = "Nota Salida"
...
Private Sub dk9893_Click()
    ...
    reporgen.NAMETABLA = "notaingreso"
    reporgen.Show 1
...
Sub ejecuta(sw As Integer)
    ...
    cad = "SELECT * from notaingreso where tipomov='" & tipomov & "'"
...
Private Sub Form_Activate()
    ...
    Frame3.Top = 20: Frame3.Left = 20

    'agregar_menus
    If tipomov = "S" Then
        tnotaing.Caption = "Nota Entrada"

    End If

    If tipomov = "T" Then
        tnotaing.Caption = "Nota Salida"
...
Sub pone_registro()
    ...
    bodega = Trim("" & txformxu.Fields("bodega"))
    'tipomov = Trim("" & txformxu.Fields("tipomov"))
...
Sub grabando()
    ...
    txformxu.Fields("bodega") = Trim(bodega)
    txformxu.Fields("tipomov") = Trim(tipomov)
    txformxu.Fields("moneda") = Trim(moneda)
    txformxu.Fields("ordentrabajo") = Val(ordentrabajo)
...
Function grabar()
    ...
    If Frame2.Caption = "Modifica" Then
        grabando
        txformxu.Update
        cn.Execute ("update notaingresod set ordentrabajo=" & Val("" & txformxu.Fields("ORDENTRABAJO")) & ",bodega='" & Trim("" & txformxu.Fields("bodega")) & "',tipomov='" & txformxu.Fields("tipomov") & "' where numero=" & txformxu.Fields("numero"))
        dlo132_Click
        Exit Function
...
Sub sumar()
    ...
    sdx1 = 0
    sdx2 = 0
    mytablex.Open "select * from notaingresod where numero=" & txformxu("numero"), cn, adOpenStatic, adLockOptimistic
    Do
        ...
        mytablex.MoveNext
    Loop
    txformxu.Fields("items") = sdx
    txformxu.Fields("total") = sdx2
    txformxu.Update
    Exit Sub
cmd86712_err:
    Exit Sub
...
Function borrar_notaingreso()
    ...
    'Do
    'If mytablex.EOF Then Exit Do
    'actualiza_stock mytablex
    'mytablex.MoveNext
    'Loop
    'mytablex.Close
End Function

Sub actualiza_stock(mytablex As ADODB.Recordset)

    Dim mytabley As New ADODB.Recordset

    mytabley.Open "select * from almacen where producto='" & "" & mytablex.Fields("producto") & "' and bodega='" & "" & mytablex.Fields("bodega") & "'", cn, adOpenStatic, adLockOptimistic

    If mytabley.RecordCount > 0 Then
        mytabley.Fields("saldo") = Val("" & mytabley.Fields("saldo")) + Val("" & mytablex.Fields("cantidad"))
        mytabley.Update
