...
Begin VB.Form tsubfami 
   BackColor       =   &H00808080&
   Caption         =   "Tabla de Subfamilias"
   ClientHeight    =   9930
   ClientLeft      =   165
   ClientTop       =   -765
   ClientWidth     =   15240
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("subfamilia"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Private Sub subfamilia_KeyPress(KeyAscii As Integer)
    ...
    If Len(subfamilia) = 0 Then Exit Sub
    descripcio.SetFocus
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from subfamil where familia='" & familia & "'"

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from subfamil   where familia='" & familia & "' and " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    subfamilia = Trim("" & txempre.Fields("subfamilia"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("familia") = Trim(familia)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select subfamilia from subfamil where familia='" & familia & "' and subfamilia='" & subfamilia & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe subfamil ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("subfamilia") = subfamilia
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
