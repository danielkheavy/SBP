...
Begin VB.Form trecibo 
   BackColor       =   &H00FFFF00&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tabla Recibos Caja"
   ClientHeight    =   8595
   ClientLeft      =   45
   ClientTop       =   615
   ClientWidth     =   13725
...
If Frame1.Caption = "NUEVO" Then
   rsexiste.Open "SELECT sede FROM sede where sede='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
   If rsexiste.RecordCount > 0 Then  'si existe
      MsgBox "Ya existe codigo ", 48, "Aviso"
      codigo.SetFocus
      Exit Sub
   End If
   cad = "INSERT INTO sede VALUES('" & Trim(codigo) & "','" & Trim(nombre) & "')"
   cn.Execute (cad)
   sql
   fdo33_Click
End If
If Frame1.Caption = "MODIFICA" Then
   cad = "UPDATE sede SET nombre = '" & Trim(nombre) & "' WHERE sede = '" & Trim(codigo) & "'"
   cn.Execute (cad)
   sql
   fdo33_Click
End If
...
If MsgBox("Desea Borrar " + dbgrid1.Columns(1), 1, "Aviso") = 1 Then
   cn.Execute ("DELETE   FROM sede WHERE sede ='" & Trim(dbgrid1.Columns(1)) & "'")
   rsede.Requery
   sql
End If
dbgrid1.SetFocus
