...
Begin VB.Form tips 
   BackColor       =   &H00808080&
   Caption         =   "Tabla de Ips de Maquinas"
   ClientHeight    =   9930
   ClientLeft      =   165
   ClientTop       =   -765
   ClientWidth     =   15240
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("ip"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        ...
        If Len(buffer) > 0 Then
            cad = "SELECT *  from ip   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    ip = Trim("" & txempre.Fields("ip"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("ip") = Trim(ip)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select ip from ip where ip='" & ip & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe ip ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("ip") = ip
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
