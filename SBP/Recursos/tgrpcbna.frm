...
Begin VB.Form tiponcd 
   BackColor       =   &H00808080&
   Caption         =   "Tipo de notas de Crédito/Débito"
   ClientHeight    =   9930
   ClientLeft      =   165
   ClientTop       =   -765
   ClientWidth     =   15240
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.Label Label4 
         Caption         =   "(NC)  > NOTA DE CRÉDITO                                                                  (ND)  > NOTA DE DÉBITO"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from TIPONCD  ORDER BY TIPO,CODIGO  "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from TIPONCD   where  " & Combo1 & " like '" & buffer & "%' ORDER BY TIPO,CODIGO "
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select CODIGO from TIPONCD where codigo='" & codigo & "'", cn, adOpenStatic, adLockOptimistic
...
Function valida()
    ...
    If Len(tipo) = 0 Then
        tipo.SetFocus
        MsgBox ("Ingrese tipo"), vbCritical
        Exit Function

    End If

    If (tipo) <> "NC" And (tipo) <> "ND" Then
        tipo.SetFocus
        MsgBox ("Ingrese tipo correcto"), vbCritical
        Exit Function
