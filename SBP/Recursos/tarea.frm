...
Begin VB.Form tarea 
   BackColor       =   &H00808080&
   Caption         =   "Tabla de Areas de Trabajo"
   ClientHeight    =   9930
   ClientLeft      =   165
   ClientTop       =   -765
   ClientWidth     =   15120
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("area"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub prueba_reporte()
    ...
    'Call Reporter(CrystalReport1, globalpath & "\001d\06\reporte\areaesproducto.rpt", "")
End Sub
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from area    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from area   where  " & Combo1 & " like '" & buffer & "%'"
...
Sub pone_registro()
    area = Trim("" & txempre.Fields("area"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("area") = Trim(area)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select area from area where area='" & area & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe area ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("area") = area
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
...
Sub agregar_menus()
    ...
    mytablex.Open "select * from archivo where menu='area' and   estado='S'", cn, adOpenStatic, adLockOptimistic
...
Sub mnuarchivoarray_click(Index As Integer)
    ...
    mytablex.Open "select * from archivo where menu='area' and descripcio='" & Trim(buf) & "'", cn, adOpenStatic, adLockOptimistic
