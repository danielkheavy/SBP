...
Begin VB.Form ttipoafi 
   BackColor       =   &H00FFFF00&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Tabla Tipos de Afiliaciones"
   ClientHeight    =   8595
   ClientLeft      =   45
   ClientTop       =   615
   ClientWidth     =   10275
...
If rsti.State = 1 Then rsti.Close
rsti.Open cad, cn, adOpenStatic, adLockOptimistic
Set dbGrid1.DataSource = rsti
...
If Frame1.Caption = "NUEVO" Then
   rsexiste.Open "SELECT tipoafilia FROM tipoafilia where tipoafilia='" & Trim(codigo) & "'", cn, adOpenKeyset, adLockOptimistic
   If rsexiste.RecordCount > 0 Then  'si existe
      MsgBox "Ya existe codigo ", 48, "Aviso"
      codigo.SetFocus
      Exit Sub
   End If
   cad = "INSERT INTO tipoafilia VALUES('" & Trim(codigo) & "','" & Trim(nombre) & "')"
   cn.Execute (cad)
   SQL
   fdo33_Click
End If
If Frame1.Caption = "MODIFICA" Then
   cad = "UPDATE tipoafilia SET nombre = '" & Trim(nombre) & "' WHERE tipoafilia = '" & Trim(codigo) & "'"
   cn.Execute (cad)
   SQL
   fdo33_Click
End If
...
If MsgBox("Desea Borrar " + dbGrid1.columns(1), 1, "Aviso") = 1 Then
   cn.Execute ("DELETE   FROM tipoafilia WHERE tipoafilia ='" & Trim(dbGrid1.columns(1)) & "'")
   rsti.Requery
   SQL
End If
dbGrid1.SetFocus
...
'rt.Open "SELECT * FROM tipoafilia ", cn, adOpenKeyset, adLockOptimistic
