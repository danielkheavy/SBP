...
Begin VB.Form tdivisio 
   BackColor       =   &H00808080&
   Caption         =   "Tabla de Divisiones"
   ClientHeight    =   9930
   ClientLeft      =   165
   ClientTop       =   -765
   ClientWidth     =   15240
...
Private Sub bo712_Click()
    ...
    If MsgBox("Desea Borra " + txempre.Fields("division"), 1, "Aviso") <> 1 Then
        Exit Sub
...
Sub prueba_reporte()
    ...
    'Call Reporter(CrystalReport1, globalpath & "\001d\06\reporte\divisionesproducto.rpt", "")
End Sub

Private Sub division_KeyPress(KeyAscii As Integer)
    ...
    If Len(division) = 0 Then Exit Sub
    descripcio.SetFocus
...
Sub ejecuta(sw As Integer)
    ...
    If opcion1 = "1" Then  'bodega
        If Len(buffer) = 0 Then
            cad = "SELECT * from division    "

        End If

        If Len(buffer) > 0 Then
            cad = "SELECT *  from division   where  " & Combo1 & " like '" & buffer & "%'"
...
Private Sub dbgrid1_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = 13 Then
        ...
        'division = dbGrid1.Columns(1)
        'Frame1.Visible = False
        'Frame1.Enabled = False
        'division.SetFocus
        'division_KeyPress 13
    End If
...
Sub pone_registro()
    division = Trim("" & txempre.Fields("division"))
    descripcio = Trim("" & txempre.Fields("descripcio"))

End Sub

Sub grabando()
    txempre.Fields("division") = Trim(division)
...
Function grabar()
    ...
    If Frame2.Caption = "Nuevo" Then
        ...
        rbusca.Open "select division from division where division='" & division & "'", cn, adOpenStatic, adLockOptimistic

        If rbusca.RecordCount > 0 Then
            rbusca.Close
            MsgBox "Ya existe division ", 48, "Aviso"
            Exit Function

        End If

        txempre.AddNew
        txempre.Fields("division") = division
        grabando
        txempre.Update
        dlo132_Click
        Exit Function
...
Sub agregar_menus()
    ...
    mytablex.Open "select * from archivo where menu='division' and   estado='S'", cn, adOpenStatic, adLockOptimistic
...
Sub mnuarchivoarray_click(Index As Integer)
    ...
    mytablex.Open "select * from archivo where menu='division' and descripcio='" & Trim(buf) & "'", cn, adOpenStatic, adLockOptimistic
